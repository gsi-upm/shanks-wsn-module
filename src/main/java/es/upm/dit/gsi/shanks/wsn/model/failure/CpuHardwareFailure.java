 ...
 * es.upm.dit.gsi.shanks.wsn.model.failure.CpuHardwareFailure.java
 * 
...
public class CpuHardwareFailure extends Failure {
	 ...
	 * @param logger
	 */
	public CpuHardwareFailure(Steppable generator, Logger logger) {
		super("CPU_Damaged_Failure_"+System.currentTimeMillis(), generator, prob, logger);
	}
	...
	public boolean isResolved() {
		...
		for (NetworkElement element : affectedElements) {
			if (element instanceof ZigBeeSensorNode) {
				...
				if (!element.getStatus().get(ZigBeeSensorNode.CPU_DAMAGED_STATUS)) {
					resolved = true;
				} else {
					resolved = false;
					break;
	...
	public void addPossibleAffected() {
		this.addPossibleAffectedElementState(ZigBeeSensorNode.class, ZigBeeSensorNode.CPU_DAMAGED_STATUS, true);
	}
