 ...
 * File: es.upm.dit.gsi.shanks.wsn.model.failure.ExternalDamageFailure.java
 * 
 ...
 * 
 */
public class ExternalDamageFailure extends Failure {
	 ...
	 * @param logger
	 */
	public ExternalDamageFailure(Steppable generator, Logger logger) {
		super("External_Damage_Failure_"+System.currentTimeMillis(), generator, prob, logger);
	}
	...
	public boolean isResolved() {
		...
		for (NetworkElement element : affectedElements) {
			if (element instanceof ZigBeeSensorNode) {
				...
				if (!element.getStatus().get(ZigBeeSensorNode.EXTERNAL_DAMAGE_STATUS)) {
					resolved = true;
				} else {
					resolved = false;
					break;
	...
	public void addPossibleAffected() {
		this.addPossibleAffectedElementState(ZigBeeSensorNode.class, ZigBeeSensorNode.EXTERNAL_DAMAGE_STATUS, true);
	}
