 ...
 * File: es.upm.dit.gsi.shanks.wsn.model.failure.SensorHardwareFailure.java
 * 
 ...
 * 
 */
public class SensorHardwareFailure extends Failure {
	 ...
	 * @param logger
	 */
	public SensorHardwareFailure(Steppable generator, Logger logger) {
		super("Sensor_Damaged_Failure_"+System.currentTimeMillis(), generator, prob, logger);
	}
	 ...
	 * @param logger
	 */
	public SensorHardwareFailure(String id, Steppable generator, double prob, Logger logger) {
		super(id, generator, prob, logger);
		// TODO Auto-generated constructor stub
	}

	/* (non-Javadoc)
	 * @see es.upm.dit.gsi.shanks.model.event.failiure.Failure#isResolved()
	 */
	@Override
	public boolean isResolved() {
		...
		for (NetworkElement element : affectedElements) {
			if (element instanceof ZigBeeSensorNode) {
				...
				if (!element.getStatus().get(ZigBeeSensorNode.SENSOR_DAMAGED_STATUS)) {
					resolved = true;
				} else {
					resolved = false;
					break;
	...
	public void addPossibleAffected() {
		this.addPossibleAffectedElementState(ZigBeeSensorNode.class, ZigBeeSensorNode.SENSOR_DAMAGED_STATUS, true);
	}
