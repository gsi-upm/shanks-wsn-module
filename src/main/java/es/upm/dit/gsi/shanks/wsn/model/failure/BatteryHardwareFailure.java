 ...
 * File: es.upm.dit.gsi.shanks.wsn.model.failure.BatteryHardwareFailure.java
 * 
 ...
 * 
 */
public class BatteryHardwareFailure extends Failure {
	 ...
	 * @param logger
	 */
	public BatteryHardwareFailure(Steppable generator, Logger logger) {
		super("Battery_Damaged_Failure_"+System.currentTimeMillis(), generator, prob, logger);
	}
	...
	public boolean isResolved() {
		...
		for (NetworkElement element : affectedElements) {
			if (element instanceof ZigBeeSensorNode) {
				...
				if (!element.getStatus().get(ZigBeeSensorNode.BATTERY_DAMAGED_STATUS)) {
					resolved = true;
				} else {
					resolved = false;
					break;
	...
	public void addPossibleAffected() {
		this.addPossibleAffectedElementState(ZigBeeSensorNode.class, ZigBeeSensorNode.BATTERY_DAMAGED_STATUS, true);
